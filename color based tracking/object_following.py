...
deadZone =100
# PID‐style parameters for forward/back control
TARGET_AREA   = 7000       # desired contour area at correct distance
KP_FB         = 0.0005    # proportional gain for forward/back
MAX_FB_SPEED  = 50         # max cm/s forward/back
######################################################################
...
global imgContour, dir, detected_area
...
while True:
    ...
    # Calculate area error and forward/backward velocity
    area_error = TARGET_AREA - detected_area
    for_back_velocity = int(np.clip(KP_FB * area_error, -MAX_FB_SPEED, MAX_FB_SPEED))

    # Clamp minimum velocity to overcome inertia
    if for_back_velocity > 0:
        for_back_velocity = max(for_back_velocity, 15)
    elif for_back_velocity < 0:
        for_back_velocity = min(for_back_velocity, -15)
    else:
        for_back_velocity = 0

    # Command persistence: update velocity every 3 frames
    frame_count += 1
    if frame_count % 3 == 0:
        me.for_back_velocity = for_back_velocity
    ...
    if dir == 1:
       ...
       me.left_right_velocity = 0; me.up_down_velocity = 0; me.yaw_velocity = 0
   # SEND VELOCITY VALUES TO TELLO
    if me.send_rc_control:
       me.send_rc_control(me.left_right_velocity, me.for_back_velocity, me.up_down_velocity, me.yaw_velocity)
    print(dir, "detected area", detected_area)
